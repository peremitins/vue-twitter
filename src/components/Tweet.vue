<template>
  <article class="tweet border-b border-gray-600">
    <router-link to="#" class="flex p-4">
      <figure class="w-16 h-16 mr-3">
        <img :src="tweet.author.profile_img_url" alt="Image" class="rounded-full w-full h-full object-cover">
      </figure>
      <section class="flex-1">
        ...
        <figure v-if="tweet.image" class="w-1/2 h-auto rounded-xl border border-gray-600 overflow-hidden">
          <img 
            :src="tweet.image.url" 
            :alt="tweet.image.alt"
